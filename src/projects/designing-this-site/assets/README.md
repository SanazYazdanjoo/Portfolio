...
`../Project-5.png` (+ `.webp` via `scripts/generate-webp.mjs`) — an
illustration generated with Google Gemini, added 2026-08-27, extended to 21:9
on a TRANSPARENT background so the md+ cover band shows the full art and the
hero's dotted paper mat shows through behind it (owner ruling: keep the
dots). It is wired as `thumbnail` in `card.js`, with `heroIsGenerated: true`
in `<slug>.data.js` rendering the visible credit, which names the tool (an AI
image must never pass as documentation).
