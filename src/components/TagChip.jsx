...
const TagChip = ({ name, count, onRemove }) => {
  return (
    <div className="group inline-flex items-center rounded-full border-[1.5px] rule-pill [--rule-line-color:var(--primary-600)] bg-transparent text-xs font-semibold tracking-wide text-primary-600 m-1
                    transition-colors duration-200 ease-smooth hover:[--rule-fill-color:var(--primary-600)] hover:[color:var(--on-primary-600)]">
      <Link to={`/tags/${encodeURIComponent(name)}`} className="px-3 py-1">
        {name}
      </Link>

      {typeof count === "number" && (
        <span className="pr-3 font-mono text-2xs text-primary-600 transition-colors duration-200 group-hover:[color:var(--on-primary-600)] group-hover:opacity-75">
          {count}
        </span>
      )}
