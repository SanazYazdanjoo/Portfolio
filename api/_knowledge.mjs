...
export default {
 "profile": {
  ...
  "profileSummary": "I'm a UX Engineer with an M.Sc. in Human-Computer Interaction (Bauhaus-Universität Weimar) and experience across frontend development, QA, and user research since 2015. I design user-centric systems that translate complex data structures and business requirements into pragmatic B2B solutions — building performant, accessible UI components in React and TypeScript, and systematically validating design decisions through mixed-methods research. I use AI-assisted development tools productively, while critically reviewing every line of generated code. The case studies below show how that plays out in real projects.",
  "impactStats": [
   {
    "value": "N=30",
   ...
   {
    "value": "Since 2015",
    "label": "Frontend, QA & research experience"
   }
  ],
  "bioParagraphs": [
   "I'm a UX Engineer with an M.Sc. in Human-Computer Interaction (Bauhaus-Universität Weimar) and experience across frontend development, QA, and user research since 2015. I design user-centric systems that translate complex data structures and business requirements into pragmatic B2B solutions: performant, accessible UI components in React and TypeScript. That mix is the point — I validate design decisions systematically through mixed-methods research, and my engineering background means I can take the findings all the way to working code. I relocated to Germany in 2021 for the degree and defended in May 2026; the QA, web, and research roles on my CV ran alongside full-time study."
  ],
  "experience": [
   {
   ...
   {
    "company": "Mega-Tech",
    "role": "UI Developer / R&D Developer",
   ...
   },
   {
    ...
    "role": "QA Engineer — Outsourced via Mega-Tech (remote)",
    ...
    "tasks": [
     "Worked as an outsourced QA engineer for Black Citrus, an Australian company, manually testing its web application projects remotely under Mega-Tech's supervision — directly after finishing the bachelor thesis."
    ]
   }
  ],
  "education": [
 ...
 "projects": [
  {
   ...
   "context": "In-house · public sector",
   "tags": [
    "UX Research",
  ...
  },
  {
   ...
   "tagline": "A recruiter gets thirty seconds. An engineer reads the source. A printer gets one page. Same site, three tests.",
   ...
   "process": [
    {
     "phase": "define",
     "type": "Design Tokens",
     "title": "Ink & Bloom — Contrast Rules Baked Into the Names",
     "annotation": "A warm paper-and-ink palette with one coral accent and a gold highlighter, defined as raw primitives and consumed only through semantic tokens and .type-* role classes — Bricolage Grotesque for display type, DM Sans for body, Caveat for hand-drawn annotation text. Primary and primary-600 aren't the same token: the -500 step is reserved for large text and shapes, the -600 step for anything small enough that WCAG AA contrast actually matters.",
     "insight": "Splitting a color into a 'large text' step and a 'small text' step means the contrast rule is enforced by which token name a component reaches for, not by a linter running after the fact — and the gold highlighter is deliberately excluded from that split, restricted by convention to a single highlighter mark per page rather than offered as a general-purpose text color."
    },
    {
     "phase": "design",
     "type": "Information Architecture",
     "title": "Three Tracks, Collapsing to One",
     "annotation": "Every case study reads as three columns above the xl breakpoint — a sticky numbered section rail, a capped-width prose column, and a right rail that lifts one pull-quote sentence out of the long-form sections — and as a single column with a horizontal pill bar below it. The right rail is explicitly documentation, not new information: below xl it simply doesn't render, because the sentence it repeats already lives in the paragraph next to it.",
     "insight": "Building the rail as a duplicate of existing text, not as its own content slot, means a translator or a future editor only has to write one sentence, not two that have to be kept in sync by hand."
    },
    {
     "phase": "design",
     "type": "Component Architecture",
     "title": "One Template, a Fixed Section List, an Escape Hatch",
     "annotation": "Every case study — this one included — renders through the same ProjectTemplate({ meta, children }) contract. A fixed SECTIONS array decides what can appear and in what order; a data key that isn't in that array simply cannot render, no matter what a data.js file contains. A children escape hatch exists for the rare case a project needs something the section list doesn't cover, but it's additive — it appends after the standard sections rather than letting a project opt out of them.",
     "insight": "The constraint is the point: this document is bound by the exact same SECTIONS list as every research case study before it, which is what makes 'built under the same rules as the case studies it documents' a checkable claim rather than a tagline."
    }
   ]
  }
 ],
